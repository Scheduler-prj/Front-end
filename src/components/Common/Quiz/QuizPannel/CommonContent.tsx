	...
	return (
		<>
			...
			<Column gap={20} horizonAlign="center" verticalAlign="center">
				{isDesktop && <NextQuizButton isQuizContent={false} isCompleted={pageType === "quizResult"} />}
				{pageType === "quiz" && (
					<B4 style={{ color: theme.colors.coolGray4, cursor: "pointer" }}>{"다음에 이어서 풀기"}</B4>
				)}
				{pageType === "quizResult" && (
					<Row gap={16} horizonAlign="right" style={{ width: "100%", color: theme.colors.coolGray2 }}>
						<Column verticalAlign="center" gap={4}>
							<S.GrayCircle />
							<B6>{"다시 풀기"}</B6>
						</Column>
						<Column verticalAlign="center" gap={4}>
